...
def describe_market_column(suffix):
    descriptions = {
        "market_ticker": "Kalshi market ticker for the favorite side.",
        "team": "Team name from Kalshi yes_sub_title for the favorite side.",
        "total_volume": "Total contracts traded in the 15-minute pre-tip window for the favorite.",
        "avg_ask_cents": "Average YES ask price (cents) across all valid 1-minute candles in the window for the favorite; used to identify the favorite and as entry price for PnL.",
    }
    if suffix.startswith("ask_min_"):
        minute = suffix[len("ask_min_"):]
        return f"YES ask close (cents) at minute {minute} of the pre-tip window for the favorite; empty if no candle."
    return descriptions.get(suffix, f"Favorite field '{suffix}' generated by the research builder.")
...
def describe_dataset_column(fieldname):
    base_descriptions = {
        ...
        "Adjusted_Tipoff_UTC": "Scheduled tipoff + 12 minutes to approximate actual tip.",
        "Window_Start_UTC": "Start of the candle window; 15 minutes before scheduled tipoff.",
        "Window_End_UTC": "End of the candle window; scheduled tipoff.",
        "Favorite_Hold_To_Settle_PnL_Cents": "Favorite per-share PnL if entered at fav_avg_ask_cents and held to settlement; (100 - entry) if won, (-entry) if lost.",
    ...
    if fieldname.startswith("fav_"):
        return describe_market_column(fieldname[len("fav_"):])
...
def summarize_market_window(candles, window_start_ts, num_minutes=PRE_TIP_LOOKBACK_MINUTES):
    total_volume = round(sum(get_candle_volume(c) for c in candles), 4)
    ...
    for c in candles:
        ...
        if end_ts is None:
            continue
        minute_idx = int((end_ts - window_start_ts) / 60)
        if 0 <= minute_idx < num_minutes:
            price = get_candle_price(c.get("yes_ask", {}), "close")
            if is_valid_quote_price(price):
                ask_by_minute[minute_idx] = price

    ask_series = [ask_by_minute.get(i) for i in range(num_minutes)]
    valid_prices = [p for p in ask_series if p is not None]
    avg_ask_cents = round(sum(valid_prices) / len(valid_prices)) if valid_prices else None

    return {
        "total_volume": total_volume,
        "avg_ask_cents": avg_ask_cents,
        "ask_series": ask_series,
    }


def choose_favorite_and_underdog(market_a, market_b, summary_a, summary_b):
    ask_a = summary_a["avg_ask_cents"]
    ask_b = summary_b["avg_ask_cents"]
    ...
    if favored_first:
        return {"favorite_market": market_a, "favorite_summary": summary_a}

    return {"favorite_market": market_b, "favorite_summary": summary_b}
...
def add_market_summary(row, prefix, market, summary):
    ...
    row[f"{prefix}_avg_ask_cents"] = summary["avg_ask_cents"]
    for i, price in enumerate(summary["ask_series"]):
        row[f"{prefix}_ask_min_{i + 1:02d}"] = price
...
def build_research_row(event_ticker, pair, game_metadata, summaries, season_date_ranges):
    ...
    favorite_entry = favorite_summary["avg_ask_cents"]
    ...
    row = {
        ...
        "Window_Start_UTC": (
            game_metadata["scheduled_tipoff_utc"] - timedelta(minutes=PRE_TIP_LOOKBACK_MINUTES)
        ).isoformat(),
        "Window_End_UTC": game_metadata["scheduled_tipoff_utc"].isoformat(),
...
def process_event_pair(event_ticker, pair, schedule_index, season_date_ranges, candle_executor):
    ...
    scheduled_tipoff = game_metadata["scheduled_tipoff_utc"]
    window_start = scheduled_tipoff - timedelta(minutes=PRE_TIP_LOOKBACK_MINUTES)
    window_end = scheduled_tipoff
...
def append_rows(csv_filename, rows):
    ...
    # Re-read existing keys at write time to guard against concurrent runs that
    # both passed the startup dedup check before either had written anything.
    existing_keys_at_write = load_existing_keys(csv_filename, "Event_Ticker")
    rows_to_write = [r for r in rows if str(r.get("Event_Ticker", "")) not in existing_keys_at_write]
